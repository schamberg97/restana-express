...
let methods = ['append', 'attachment', 'cookie', 'clearCookie', 'download', 'format', 'get',
	'json', 'links', 'location', 'redirect', 'render', 'sendFile', 'sendStatus', 'set',
	'status', 'type', 'vary'
]
...
expressRes_.render = function (inp, options, callback) {
	...
	if (!this.__restanaExpressResOptions.render.viewsDir) {
		return this.status(500).json({code:500, message: `Could not render ${inp}, because res.render is not configured correctly in restana-express middleware`, settings: this.__restanaExpressResOptions.render})
	}
	let done = callback;
	let opts = options || {};
	...
	let res = this
		...
		if (err) {
			...
			if (process.env.NODE_ENV !== "production") {
				...
				err = err.toString()
				return res.status(status).json({code:status, message:err})
			}
