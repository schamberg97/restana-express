...
expressReq.stalePropFn = function() {
	if (this.fresh instanceof Function) {
		return this.freshPropFn()
	}
	return !this.fresh
}
expressReq.securePropFn = function() {
	if (this.protocol instanceof Function) {
		let val = this.protocolPropFn()
		return val === 'https'
	}
	...
	reqToUse.forEach(name => {
		...
		if (index !== -1) {
			if (name.includes('query')) {
				expressReqFinal.query_ = compileQueryParser(reqOptions.queryParser)
				if (expressReqFinal.query_ === "no-need") delete expressReqFinal.query_
				return
			}
