	...
	reqToUse.forEach(name => {
		...
		if (index !== -1) {
			if (name.includes('query')) {
				expressReqFinal.queryPropFn = compileQueryParser(reqOptions.queryParser)
				if (expressReqFinal.queryPropFn === "no-need") delete expressReqFinal.queryPropFn
				return
			}
...
function compileQueryParser(val) {
	...
	if (typeof val === 'function') {
	  let newFunction = function() {
		let toParse = parseUrl(this).query
		return val(toParse)
	  }
	  return newFunction;
	}
	...
	switch (val) {
		...
		case 'extended':
			fn = function(req,res) {
				let toParse = parseUrl(this).query
				return parseExtendedQueryString(toParse)
			}
			break;
		case 'simple':
		  fn = 'no-need';
